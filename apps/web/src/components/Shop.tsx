  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col bg-black/60 backdrop-blur-[3px] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col p-3 overflow-y-auto relative z-10" style={{ paddingTop: "calc(env(safe-area-inset-top) + 12px)" }}>
        <div className="font-pixel-fat text-3xl txt-chrome text-center mb-2">{dict.shopTitle[lang]}</div>
        <div className="font-pixel text-base text-[#ff9e2c] text-center mb-3 font-bold">💰 ${money}</div>

      {/* For Sale header */}
      <div className="flex items-center justify-between mb-1.5">
        <span className="font-pixel text-sm font-bold text-gray-300">{dict.forSale[lang]}</span>
      </div>
      ...
      <div className="flex flex-col gap-2.5 mb-4">
        {offers.map((def) => {
          ...
          return (
            <div key={def.id} className="panel rounded-xl p-2.5 flex items-center gap-2.5">
              <div className="w-11 h-14 shrink-0 rounded-md overflow-hidden flex items-center justify-center relative p-0.5">
                <JokerArtworkFrame rarity={def.rarity} className="h-full w-full" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-pixel-fat text-base text-white leading-none">{jokerName(def, lang)}</div>
                <div style={{ color: RARITY_COLOR[def.rarity] }} className="font-pixel text-xs leading-none mt-1 font-bold capitalize">{rarityName(def.rarity, lang)}</div>
                <div className="font-pixel text-xs text-gray-200 leading-snug mt-1.5">{jokerDesc(def, lang)}</div>
                {conflict && (
                  <div className="font-pixel text-[11px] text-[#ff2e88] leading-tight mt-1">
              ...
              <button
                ...
                onClick={() => onBuy(def)}
                className="btn-chunky btn-green shrink-0 px-3 py-1.5 text-sm font-bold"
              >
      ...
      <div className="mb-2.5 mt-1.5 rounded-xl border border-[#b026ff]/30 bg-[#1a0d3a]/70 backdrop-blur-sm p-2.5">
        <div className="font-pixel text-sm font-bold text-[#c9b8ff] mb-2 text-center">{dict.boosterPacks[lang]}</div>
              ...
              >
                <div className="pack-body">
                  ...
                  >
                    ...
                    <span
                      className="font-pixel text-[9px] font-bold mt-0.5 z-10 leading-none uppercase tracking-wider"
                ...
                <span className="font-pixel text-xs font-bold text-[#ff9e2c] leading-none mt-1">$0.02</span>
              </button>
            );
          })}
        </div>
        ...
        {activePack !== null && (isBusy || packState === "success" || packState === "error") && (
          <div className="font-pixel text-xs font-bold text-center mt-2 leading-tight"
        ...
        {packResult && (
          <div className={`mt-2 rounded-md p-2 flex items-center gap-2 anim-pop mx-auto max-w-[270px] ${
            packResult.duplicate ? "bg-[#2a0d5a] border border-[#ff9e2c]/30" : "bg-[#0a2a24] border border-[#00f0ff]/40"
          }`}>
            <div className="w-9 h-11 rounded overflow-hidden flex items-center justify-center shrink-0 p-0.5">
              <JokerArtworkFrame rarity={packResult.rarity} className="h-full w-full" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-pixel-fat text-base text-white leading-none">{packResult.name}</div>
              <div style={{ color: RARITY_COLOR[packResult.rarity] }} className="font-pixel text-xs leading-none mt-1 font-bold capitalize">
                {rarityName(packResult.rarity, lang)}
              </div>
              <div className="font-pixel text-[11px] text-gray-300 mt-1">
        ...
        {packError && (
          <div className="font-pixel text-xs text-[#ff2e88] text-center mt-1 leading-tight font-bold">
            {packError}
          </div>
        )}

        <div className="font-pixel text-[10px] text-white/70 leading-tight flex gap-3 flex-wrap justify-center mt-2">
      ...
      {ownedJokers.length > 0 && (
        <>
          <div className="font-pixel text-sm font-bold text-gray-300 mb-1.5">{dict.yourJokersSell[lang]}</div>
          <div className="flex gap-2 flex-wrap mb-4">
            {ownedJokers.map((oj, idx) => (
              <button
                ...
                type="button"
                onClick={() => onSell(idx)}
                className="panel tap-target rounded-xl p-1.5 flex flex-col items-center gap-1 w-[68px] text-center hover:brightness-110 active:scale-95 transition-transform"
                title={fmt(dict.sellFor[lang], { amount: Math.floor(jokerBaseCost(oj.def) / 2) })}
              >
                <div className="w-9 h-11 rounded overflow-hidden flex items-center justify-center relative p-0.5">
                  <JokerArtworkFrame rarity={oj.def.rarity} className="h-full w-full" />
                </div>
                <div className="font-pixel text-[10px] text-gray-200 leading-none truncate w-full">{jokerName(oj.def, lang)}</div>
                <div className="font-pixel text-xs font-bold text-[#ff9e2c]">${Math.floor(jokerBaseCost(oj.def) / 2)}</div>
              </button>
            ))}
          </div>
        </>
      ...
      <div className="font-pixel text-[11px] text-white/75 text-center mb-2 leading-snug px-2">
        {dict.packInfo[lang]}
      </div>
